...
def matchmaking(job,compute_nodes):
    ...
    :param compute_nodes:
    :return: (node,job). The node where this job has to be scheduled, and also the job that is to be preempted (if any)
    """
    ...
    if(len(candidates) > 0):
        ...
        # Preemption is needed, so first first the lowest priority job on each machine
        lowest_priority_jobs = {}
        for node_id, job_list in running_jobs.items():
            lowest_priority_jobs[node_id] = min(job_list, key=lambda j: j.priority)
        ...
        for node_id, lowest_priority_job in lowest_priority_jobs.items():
            if job.min_memory < compute_nodes[node_id][memory] + lowest_priority_job.min_memory:
                if (job_to_preempt is None) or (job_to_preempt.priority > lowest_priority_job.priority):
